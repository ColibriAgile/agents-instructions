...
> Esta seção alimenta diretamente a skill `criar-tasks` — cada linha deve virar um teste concreto (automatizado ou manual) mapeado a um requisito do PRD.
...
> Compatível com o critério de ordenação da `criar-tasks` ("dependências antes das dependentes") — aqui a dependência é por risco: extrair lógica testável primeiro, mexer em UI/controles por último.
