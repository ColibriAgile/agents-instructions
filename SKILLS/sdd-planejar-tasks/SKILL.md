...
3. Use fatias verticais: um resultado revisável, implementação e testes na mesma task. Separe fundação apenas quando desbloquear múltiplas entregas ou permitir migração independente. Modele dependências acíclicas e colisões de arquivos/contratos; numere novas tasks após o maior ID na raiz e em `done/`. As tasks rodam uma por vez numa única sessão escritora: dimensione cada uma para caber numa sessão com folga para exploração, testes e revisão, e nomeie em `Contexto a recuperar sob demanda` as poucas fontes de que uma sessão nova precisa.
