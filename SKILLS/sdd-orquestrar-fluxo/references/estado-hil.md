...
4. Com operações concluídas e gravação conferida, marque `safe_to_stop: true` e faça a pausa de sessão de `continuidade-sessao.md`: abaixo do limiar de contexto e sem gate, ela segue sem perguntar; ao parar, grava o snapshot antes da pergunta. A regra de independência dela vale antes da revisão. Combine a pergunta com HIL já necessário; entre tasks, ela trata só de continuidade, sem repetir aprovação técnica. Gate e escolha de sessão são independentes: pausar não equivale a aprovar. Registre respostas e salve novamente; snapshot gravado entra em `sources.snapshot` e `next_action.read`. Silêncio mantém o gate/continuidade aguardando, sem iniciar nova unidade.
5. Se a escolha encerrar a sessão, grave `status: pausado`, preserve `pending_hil` se ainda não respondido e informe o caminho do checkpoint; o comando de retomada impresso pela pausa nomeia o fluxo com `--prd <slug>`. Encerre o turno sem novos exploradores ou processos. Se continuar, por escolha ou no destino `Seguir`, grave `ativo` e prossiga só com autorização válida; antes de gravar, atualize `safe_to_stop: false` e registre handles assim que exploradores ou processos iniciarem.

**Concluído quando:** checkpoint foi relido, fontes e próxima ação conferidas, nenhuma gravação, explorador ou processo permanece ativo numa pausa anunciada como segura. Não prometa evitar compactação ou troca automática de sessão; a skill sugere a pausa pelo limiar, e encerrar é escolha humana. A criação de checkpoint não significa conclusão da feature.
