...
- Referência: `ColibriAgile/nota-fiscal`, branch `master`, commit `6b8c1e4`.
- Alvo: consumidores e futuros pontos de refatoração no mesmo repositório, principalmente `NotaFiscal/**` e `_lib/**`; não existe alvo separado informado.
...
| --- | --- | --- | --- | --- | --- |
| E1 | O padrão pertence ao repositório atual e a referência principal é `CoLib.Logging` | `_lib/CoLib.Logging/`, commit `6b8c1e4` | Premissa aceita — o usuário delimitou a investigação ao repositório atual; não foi fornecido alvo externo | alta | Contexto de R1-R12 e G1-G10 |
...
| E4 | Opções padrão incluem rolling diário, `.logc`, layout, shared file e manutenção | `_lib/CoLib.Logging/Configuration/LogConfigurationOptions.cs`; `_lib/CoLib.Logging/Configuration/LoggingSettings.json` | Defaults observados: `RollingInterval.Day`, `SharedFile=true`, `EnableDailyZipRotation=true`, auditoria e template com `|` | alta | R3, R4, R8, G4, G6 |
| E5 | Configuração de aplicação confirma nomes e layout concretos | `NotaFiscal/Api/LoggingSettings.json`; `NotaFiscal/Api/notafiscal.csproj` | API copia `LoggingSettings.json`, usa `{ApplicationName}_{Date}.logc`, subpasta JSON `Notas` e template delimitado | alta | R2-R4, R8, G1, G4 |
...
| E7 | Entry points usam subpastas e nomes explícitos por aplicação | `NotaFiscal/Api/Program.cs`, bloco `UseLogging`; `NotaFiscal.Config/Program.cs`, bloco `AddLogging` | API usa `NotaFiscal.Api`/`NF.Api`; Config usa `NotaFiscal.Config`/`NF.Config` | alta | R1-R3, G1 |
...
| E11 | Consumidores usam `descrição|detalhes` com variáveis, XML, status e payload | `NotaFiscal.Business/Client.cs`; `NotaFiscal.Business/NFCe/NotaFiscalNFCe.cs`; `NotaFiscal.Business/Sefaz/SefazSoapClient.cs`; `NotaFiscal.Data/DataNF.cs`; `NotaFiscal/Api/Services/XmlExportV2JobManager.cs` | Há exemplos como `Request Soap XML|{RequestXml}`, `...|JobId={JobId}` e `...|{XmlCompleto}` | alta | R5-R7, G2-G3 |
| E12 | Middleware interno define formato e filtros de request/response | `_lib/CoLib.AspNet/Middlewares/RequestResponseLoggingMiddleware.cs`; `CoLib.Library.Testes/Source/AspNet/RequestResponseLoggingMiddlewareTestes.cs`; `NotaFiscal/Api/Program.cs` | Usa `→ ...|body` e `← ...|body`; omite HTML, ignora estáticos, respeita `load_tests`/atributo e tem testes de JSON/HTML | alta | R9, G7 |
| E13 | DI registra um único provider Serilog | `_lib/CoLib.Logging/Extensions/ServiceCollectionExtensions.cs`; `NotaFiscal/Api/Program.cs`; `NotaFiscal.Config/Program.cs` | Consumidores resolvem `ILogger<T>` depois do registro das extensões internas | alta | R1, G5 |
...
| E18 | Existem divergências legadas no alvo | `NotaFiscal.Business/NFCe/NotaFiscalNFCe.cs`, usos de `LogError(ex.Message)` e `$"..."`; `_lib/CoLib.Cache/.../GerenciadorDeCacheJson.cs` | A prática atual não é 100% uniforme; a orientação não deve declarar conformidade total do legado | alta | R6-R7, G3; lacuna |
| E19 | Há deriva de versões entre a biblioteca e consumidores | `_lib/CoLib.Logging/CoLib.Logging.csproj`; `NotaFiscal.Business/NotaFiscal.Business.csproj`; `NotaFiscal.Data/NotaFiscal.Data.csproj`; `NotaFiscal/Api/notafiscal.csproj` | Microsoft.Extensions aparece em `10.0.10` na biblioteca e `10.0.11` em consumidores; API também referencia `Serilog.AspNetCore 10.0.0` | alta | R1, G5; escalonamento |
| E20 | Configuração fluentemente definida pode sobrescrever JSON autodetectado | `LogConfigurationBuilder` construtor, `TryLoadAutoDetectConfigurationFile`, `MergeOptions`; `NotaFiscal/Api/Program.cs` | API contém `LogSubFolderName=Notas` no JSON, mas o entry point aplica `NotaFiscal.Api` depois | alta | R2, R4, G1; adaptação |
| E21 | O alvo operacional é o mesmo repositório, não uma aplicação separada | Escopo da solicitação e árvore `D:\Projetos\nf` | Foi adotada a premissa explícita de orientar refatorações futuras dos consumidores do próprio repositório | média | Contexto; lacuna de paridade |
| E22 | A API cria um logger temporário de console antes do host | `NotaFiscal/Api/Program.cs`, bloco `LoggerFactory.Create` próximo da configuração de CORS | O logger local é usado para construir `ConfiguracoesDeAmbiente` antes de `builder.Build`; é uma exceção de bootstrap, não o sink principal | alta | G9; escalonamento |
| E23 | O executável Audit usa console simples, não `CoLib.Logging` | `NotaFiscal.Audit/Program.cs`, `LoggerFactory.Create`; `NotaFiscal.Audit/NotaFiscal.Audit.csproj` | Usa `AddSimpleConsole`, `Microsoft.Extensions.Logging.Console` e `Spectre.Console`; não há `ProjectReference` para `CoLib.Logging` | alta | G9; exceção operacional |
...
| `LoggingSettings.json` e configuração fluentemente definida coexistem | A subpasta efetiva pode diferir do valor no JSON | Manter precedência documentada; validar o caminho efetivo no entry point |
...
| Há executáveis fora do padrão compartilhado | `NotaFiscal.Audit` e o logger temporário da API podem produzir saída apenas no console | Preservar como exceção delimitada ou aprovar uma migração explícita para `.logc` e `CoLib.Logging` |
...
- `NotaFiscal.Audit` e o logger temporário da API foram tratados como exceções observadas, não como padrão recomendado.
